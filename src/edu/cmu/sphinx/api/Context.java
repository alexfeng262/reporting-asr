...
import static edu.cmu.sphinx.util.props.ConfigurationManagerUtils.showConfig;
import static edu.cmu.sphinx.util.props.ConfigurationManagerUtils.toXML;
...
public class Context {
        ...
        throws IOException, MalformedURLException
    {
        ...
        if (null != config.getNewConfig() ){
            System.out.println("SETTING NEW CONFIG");
            setNewConfig(config.getNewConfig());
        ...
        showConfig(configurationManager);
        //System.out.print(toXML(configurationManager));
    }
    ...
    public void setNewConfig( Map<String, String> newConfig){
        ...
        for (String key : newConfig.keySet()) {
                System.out.println(key +" --- " + newConfig.get(key));
                
                switch(key){
                    case "relativeBeamWidth":
                        setLocalProperty("wordPruningSearchManager->relativeBeamWidth",newConfig.get(key));
                        setLocalProperty("wordPruningLookaheadSearchManager->relativeBeamWidth",newConfig.get(key));
                        setLocalProperty("activeList->relativeBeamWidth",newConfig.get(key));
                        setLocalProperty("fastmatchActiveList->relativeBeamWidth",newConfig.get(key));
                        setLocalProperty("standardActiveListFactory->relativeBeamWidth",newConfig.get(key));
                        setLocalProperty("wordActiveListFactory->relativeBeamWidth",newConfig.get(key));                  
                        break;
                        
                    case "wordInsertionProbability":
                        setLocalProperty("lexTreeLinguist->wordInsertionProbability",newConfig.get(key));
                        setLocalProperty("alignerLinguist->wordInsertionProbability",newConfig.get(key));
                        setLocalProperty("flatLinguist->wordInsertionProbability",newConfig.get(key));                        
                        break;
                        
                    case "languageWeight":
                        setLocalProperty("alignerLinguist->languageWeight",newConfig.get(key));
                        setLocalProperty("flatLinguist->languageWeight",newConfig.get(key));
                        setLocalProperty("lexTreeLinguist->languageWeight",newConfig.get(key));
                        break;
               ...
               //setGlobalProperty(key,newConfig.get(key));  
            }
